...
# 这是提取后的小数据集存放路径
OUTPUT_PATH = r"d:\project6\cnwordextract\HWDB1.1\subset_1000"
WORKER_COUNT = 8  # 进程数
...
# 1. 提取 target_chars_str 中的所有中文字符
for char in target_chars_str:
    # 判断是否为中文字符 (简陋版范围，但对常用字足够)
    if '\u4e00' <= char <= '\u9fff': 
        target_chars.add(char)

# 2. 强制补全核心数字和方位 (防止用户列表遗漏)
# 二, 六, 七, 九, 零, 亿
mandatory_chars = "一二三四五六七八九十零百千万亿上下左右东南西北"
for char in mandatory_chars:
    target_chars.add(char)

print(f"目标提取字符数 (去重后): {len(target_chars)}")
# ===========================================
